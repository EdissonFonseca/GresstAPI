...
/// <summary>
/// Solicitud with detail data (current state only). No order/planning fields;
/// those are joined in the application layer when building transport/collections views.
/// Stage and Phase are process-oriented (per waste_management_states.docx); set by Infrastructure when mapping from IdEtapa/IdFase.
/// </summary>
public class SolicitudWithDetailsDto
{
    ...
    // Detail (0 for solicitud-without-detail rows)
    ...
    /// <summary>Process stage (from waste_management_states.docx). Set by Infrastructure from IdEtapa.</summary>
    public RequestFlowStage? Stage { get; set; }

    /// <summary>Process phase within stage. Set by Infrastructure from IdFase.</summary>
