...
/// <summary>
/// Service for managing Providers (Personas with Provider role)
/// Filters Personas by IdRol = "PR" (or similar provider role code)
/// </summary>
public class ProviderService : IProviderService
{
    ...
    private const string PROVIDER_ROLE_CODE = "PR"; // Ajustar según los valores reales en BD
    ...
    {
        var persons = await _personRepository.GetByRoleAsync(PROVIDER_ROLE_CODE, cancellationToken);
    ...
    public async Task<IEnumerable<ProviderDto>> GetByDocumentNumberAsync(string documentNumber, CancellationToken cancellationToken = default)
    {
        var persons = await _personRepository.GetByRoleAsync(PROVIDER_ROLE_CODE, cancellationToken);
    ...
    public async Task<ProviderDto> CreateAsync(CreateProviderDto dto, CancellationToken cancellationToken = default)
    {
        ...
        await _personRepository.SetPersonRoleAsync(person.Id, PROVIDER_ROLE_CODE, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        return MapToDto(person);
    }

    public async Task<ProviderDto?> UpdateAsync(UpdateProviderDto dto, CancellationToken cancellationToken = default)
    {
        // Verificar que sea proveedor
        var person = await _personRepository.GetByIdAndRoleAsync(dto.Id, PROVIDER_ROLE_CODE, cancellationToken);
        if (person == null)
            return null;
        ...
        var updatedPerson = await _personRepository.GetByIdAndRoleAsync(dto.Id, PROVIDER_ROLE_CODE, cancellationToken);
    ...
    private ProviderDto MapToDto(Person person)
    {
